...
from unlabel2label_final import StandaloneCode as BaseStandaloneCode
...
class StandaloneCode(BaseStandaloneCode):
    # dict.get()：返回指定键的值，如果键不在字典中返回默认值 None 或者设置的默认值
    def __init__(self, conf=None):
        super(StandaloneCode, self).__init__(conf)
    ...
    def train(self, model):
        if self.train_params["reload"] > 0:
            self.loadModelEpoch(
                model, self.train_params["reload"], 0.5, 0.5, 0.5, 0.5, 0.0006
            )

        d12, d3, d4, d5, r = (
            self.train_params["dropout1"],
            self.train_params["dropout3"],
            self.train_params["dropout4"],
            self.train_params["dropout5"],
            self.train_params["regularizer"],
        )
        batch_size = self.train_params.get("batch_size", 100)  # 100
        nb_epoch = self.train_params.get("nb_epoch", 20)  # 2000

        val_loss = {"loss": 1.0, "epoch": 0}
        ...
        for i in range(self.train_params["reload"] + 1, nb_epoch):
            ...
            text_S1, text_S2, code, queries, labels, _ = self.getData(
            ...
            print(
                "Best: Loss = {}, Epoch = {}".format(
                    val_loss["loss"], val_loss["epoch"]
                )
            )
            # loss precision,recall,f1,accuracy

            train_acc, train_f1, train_recall, train_precision, train_loss = self.valid(
            ...
            print(
                "train data: %d loss=%.3f, acc=%.3f,  precision=%.3f,  recall=%.3f,  f1=%.3f,  "
                % (i, train_loss, train_acc, train_precision, train_recall, train_f1)
            )
            dev_acc, dev_f1, dev_recall, dev_precision, loss = self.valid(
                model, self.data_params["valid_path"]
            )
            ...
            print(
                "dev data: %d loss=%.3f, acc=%.3f,  precision=%.3f,  recall=%.3f,  f1=%.3f,  "
                % (i, loss, dev_acc, dev_precision, dev_recall, dev_f1)
            )
            test_acc, test_f1, test_recall, test_precision, loss = self.valid(
                model, self.data_params["test_path"]
            )
            ...
            print(
                "test data: %d loss=%.3f, acc=%.3f,  precision=%.3f,  recall=%.3f,  f1=%.3f,  "
        ...
        print(
            "最大dev f1由 %d-th epoch: precision=%.3f, recall=%.3f, f1=%.3f, accuracy=%.3f"
            % (
                max_idx,
                epoch_dev_precision[max_idx],
                epoch_dev_recall[max_idx],
                epoch_dev_f1[max_idx],
                epoch_dev_accuracy[max_idx],
            )
        )
        print(
            "相应的测试性能: precision=%.3f, recall=%.3f, f1=%.3f, accuracy=%.3f"
            % (
                epoch_test_precision[max_idx],
                epoch_test_recall[max_idx],
                epoch_test_f1[max_idx],
        ...
        params = (
            "记录:dropout12=%.3f,dropout3=%.3f,dropout4=%.3f,dropout5=%.3f,num =%.5f"
            % (d12, d3, d4, d5, r)
        )
        ...
        f.writelines("\n")
        f.writelines(
            "最大test f1由 %d-th epoch:f1: precision=%.3f, recall=%.3f, f1=%.3f, accuracy=%.3f"
            % (
                max_idx_t,
                epoch_test_precision[max_idx_t],
                epoch_test_recall[max_idx_t],
                epoch_test_f1[max_idx_t],
    ...
    def valid(self, model, path):
        ...
        labelpred = model.predict(
            [np.array(text_S1), np.array(text_S2), np.array(code), np.array(queries)],
            batch_size=100,
        )
...
def parseArgs():
    ...
    parser.add_argument(
        "--train", choices=["python", "sql"], default="python", help="train dataset set"
    )
    parser.add_argument(
        "--mode",
        choices=["train", "eval"],
    ...
    parser.add_argument(
        "--verbose", action="store_true", default=True, help="Be verbose"
    )
...
if __name__ == "__main__":
    ...
    model = eval(conf["model_params"]["model_name"])(
        conf
    )  # initialize the model,  model== <models.CARLCS_CNN object at 0x7f1d9c2e2cc0>
    ...
    if args.mode == "train":
        StandaloneCode.train(model, train_path)

    elif args.mode == "eval":
        # 加载模型python
        StandaloneCode.loadModelEpoch(model, 121, 0.5, 0.5, 0.5, 0.5, 0.0006, r)
        # 加载模型sql
        StandaloneCode.loadModelEpoch(model, 83, 0.25, 0.25, 0.25, 0.25, 0.0006, r)
        # 测试集评估
        StandaloneCode.eval(model, test_path, "test")
