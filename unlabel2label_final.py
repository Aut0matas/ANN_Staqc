...
from configs import getConfig_u2l
from DataProcess.paths import SQL_PATHS, TEXT_BLOCK_LENGTH
...
class StandaloneCode:
    ...
    def __init__(self, conf=None):
        ...
        self._buckets = conf.get(
            "buckets",
            [(2, 10, 22, 72), (2, 20, 34, 102), (2, 40, 34, 202), (2, 100, 34, 302)],
        )
        self._buckets_text_max = (
            max([i for i, _, _, _ in self._buckets]),
            max([j for _, j, _, _ in self._buckets]),
        )
        self._buckets_code_max = (
            max([i for _, _, i, _ in self._buckets]),
            max([j for _, _, _, j in self._buckets]),
        )
    ...
    def pad(data, maxlen=None):
        return pad_sequences(
            data, maxlen=maxlen, padding="post", truncating="post", value=0
        )

    # Model Loading / saving
    def saveModelEpoch(self, model, epoch, d12, d3, d4, d5, r):
        if not os.path.exists(
            self.path + "models/" + self.model_params["model_name"] + "/"
        ):
    ...
    def loadModelEpoch(self, model, epoch, d12, d3, d4, d5, r):
        print(self.path)
        print(
            "{}/pysparams:d12={}_d3={}_d4={}_d5={}_r={}_epo={:d}_class.h5".format(
    ...
    def getData(self, path):
        ...
        text_blocks = self.processMatrix(
            np.array([samples_term[1] for samples_term in data]), TEXT_BLOCK_LENGTH
        )
        text_S1, text_S2 = text_blocks[0], text_blocks[1]

        code_blocks = self.processMatrix(
            np.array([samples_term[2] for samples_term in data]), TEXT_BLOCK_LENGTH - 1
        )
        code = code_blocks[0]
...
if __name__ == "__main__":
    ...
    # ---------有标签的地址----------
    hnn_label_sql_path = (
        "../data_processing/hnn_process/ulabel_data/staqc/hnn_label_sql.txt"
    )
    # staqc:存放only-code、only-text、codemf标签地址
    staqc_sql_final_label = (
        "../data_processing/hnn_process/ulabel_data/staqc/sql_final_label.txt"
    )
    # staqc:利用only-code、only-text、codemf中都为1筛选后的语料地址
    save_path_final_label_staqc_sql = (
        "../data_processing/hnn_process/ulabel_data/staqc/combine_final_sql_label.txt"
    )

    # large:存放only-code、only-text、codemf标签地址
    large_sql_final_label = (
        "../data_processing/hnn_process/ulabel_data/staqc/large_sql_final_label.txt"
    )
    ...
    large_python_f = (
        "../data_processing/hnn_process/ulabel_data/large_corpus/multiple"
        "/seri_python_large_multiple_unlabeled.pkl"
    )
    large_single_python_path = (
        "../data_processing/hnn_process/ulabel_data/large_corpus/single"
        "/python_large_single_label.txt"
    )
    # ---------有标签的地址----------
    hnn_label_python_path = (
        "../data_processing/hnn_process/ulabel_data/staqc/hnn_label_python.txt"
    )
    # staqc:存放only-code、only-text、codemf标签地址
    staqc_python_final_label = (
        "../data_processing/hnn_process/ulabel_data/staqc/python_final_label.txt"
    )
    # staqc:利用only-code、only-text、codemf中都为1筛选后的语料地址
    save_path_final_label_staqc_python = "../data_processing/hnn_process/ulabel_data/staqc/combine_final_python_label.txt"

    # large:存放only-code、only-text、codemf标签地址
    large_python_final_label = (
        "../data_processing/hnn_process/ulabel_data/staqc/large_python_final_label.txt"
    )
    # large:利用only-code、only-text、codemf中都为1筛选后的语料地址
    ...
    if args.mode == "eval":
        ...
        finalAnalyse(
            staqc_sql_final_label, hnn_label_sql_path, save_path_final_label_staqc_sql
        )
