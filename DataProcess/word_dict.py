...
def get_vocab(corpus1, corpus2):
    ...
    for i in range(0, len(corpus1)):
        ...
        for j in range(0, len(corpus1[i][2][0])):  # len(corpus2[i][2])
            word_vacab.add(
                corpus1[i][2][0][j]
            )  # 注意之前是 word_vacab.add(corpus2[i][2][j])
...
if __name__ == "__main__":
    ...
    sql_hnn = (
        "/home/gpu/RenQ/stqac/data_processing/hnn_process/data/sql_hnn_data_teacher.txt"
    )
    sql_staqc = (
        "/home/gpu/RenQ/stqac/data_processing/hnn_process/data/staqc/sql_staqc_data.txt"
    )
