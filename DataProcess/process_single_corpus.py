...
def data_staqc_prpcessing(filepath, save_single_path, save_mutiple_path):
    with open(filepath, "r") as f:
        ...
        f.close()
    qids = [total_data[i][0][0] for i in range(0, len(total_data))]
    result = Counter(qids)
...
def single_unlable2lable(path1, path2):
    ...
    labels = [[total_data[i][0], 1] for i in range(0, len(total_data))]
...
if __name__ == "__main__":
    ...
    staqc_python_path = (
        "../hnn_process/ulabel_data/python_staqc_qid2index_blocks_unlabeled.txt"
    )
    staqc_python_sigle_save = (
        "../hnn_process/ulabel_data/staqc/single/python_staqc_single.txt"
    )
    staqc_python_multiple_save = (
    ...
    large_python_path = (
        "../hnn_process/ulabel_data/python_codedb_qid2index_blocks_unlabeled.pickle"
    )
    large_python_single_save = (
        "../hnn_process/ulabel_data/large_corpus/single/python_large_single.pickle"
    )
    large_python_multiple_save = (
        "../hnn_process/ulabel_data/large_corpus/multiple/python_large_multiple.pickle"
    )
    data_large_prpcessing(
        large_python_path, large_python_single_save, large_python_multiple_save
    )

    # 将large_sql中的单候选和多候选分开
    large_sql_path = (
        "../hnn_process/ulabel_data/sql_codedb_qid2index_blocks_unlabeled.pickle"
    )
    large_sql_single_save = (
        "../hnn_process/ulabel_data/large_corpus/single/sql_large_single.pickle"
    )
    large_sql_multiple_save = (
        "../hnn_process/ulabel_data/large_corpus/multiple/sql_large_multiple.pickle"
    )
    # data_large_prpcessing(large_sql_path, large_sql_single_save, large_sql_multiple_save)

    large_sql_single_label_save = (
        "../hnn_process/ulabel_data/large_corpus/single/sql_large_single_label.txt"
    )
    large_python_single_label_save = (
        "../hnn_process/ulabel_data/large_corpus/single/python_large_single_label.txt"
    )
