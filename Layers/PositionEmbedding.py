...
class PositionEmbedding(Layer):
    ...
    def call(self, x):  # 上一层一般就是embedding层，batch_size,seq_len,model_dim
        ...
        position_j = 1.0 / backend.pow(
            10000.0, 2 * backend.arange(self.size / 2, dtype="float32") / self.size
        )  #
        ...
        position_i = (
            backend.cumsum(backend.ones_like(x[:, :, 0]), 1) - 1
        )  # backend.arange不支持变长，只好用这种方法生成
        ...
        position_ij_2i = backend.sin(position_ij)[
            ..., tf.newaxis
        ]  # bs,seq_len,model_dim/2,1
        position_ij_2i_1 = backend.cos(position_ij)[
            ..., tf.newaxis
        ]  # bs,seq_len,model_dim/2,1
        position_ij = backend.concatenate(
            [position_ij_2i, position_ij_2i_1]
        )  # bs,seq_len,model_dim/2,2
        position_ij = backend.reshape(
            position_ij, (batch_size, seq_len, self.size)
        )  # bs,seq_len,model_dim
        # position_ij = backend.concatenate([backend.cos(position_ij), backend.sin(position_ij)], 2)#这个实现没有交叉拼接，前半部分都用的cos，后半部分都用的sin
        if self.mode == "sum":
