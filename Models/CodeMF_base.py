...
from Layers.MultiHeadAttention import MultiHeadAttention_
from Layers.PositionEmbedding import PositionEmbedding
from Layers.PositionWiseFeedForward import PositionWiseFeedForward
from Layers.StackingLayer import StackingLayer
from Models.CodeMF_base import CodeMF as BaseCodeMF
...
class CodeMF:
    def __init__(self, config):
        ...
        self.queries = Input(
            shape=(self.queries_length,), dtype="int32", name="i_queryname"
        )
        ...
        self.text_embbeding = pickle.load(
            open(self.data_params["text_pretrain_emb_path"], "rb"),
            encoding="iso-8859-1",
        )
        self.code_embbeding = pickle.load(
            open(self.data_params["code_pretrain_emb_path"], "rb"),
            encoding="iso-8859-1",
        )
    ...
    def build(self):
        ...
        attention_trans_layer = Lambda(
            lambda x: backend.permute_dimensions(x, (0, 2, 1)), name="trans_attention"
        )
    ...
    def fit(self, x, y, **kwargs):
        assert (
            self.class_model is not None
        ), "Must compile the model before fitting data"
    ...
    def save(self, class_model_file, **kwargs):
        assert (
            self.class_model is not None
        ), "Must compile the model before saving weights"
    ...
    def mycrossentropy(self, y_true, y_pred, e=0.1):
        ...
        loss2 = backend.categorical_crossentropy(
            backend.ones_like(y_pred) / self.nb_classes, y_pred
        )
